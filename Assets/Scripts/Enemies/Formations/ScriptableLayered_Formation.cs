...
[CreateAssetMenu(fileName = "Layered", menuName = "Formations/Layered")]

public class ScriptableLayered_Formation : Scriptable_FormationBase
{
    public Layered_Formation formation;
...
public class Layered_Formation : Formation_Base
{
    ...
    public override Stack<int> SetUp(ref Stack<int> occuredBursts, ref Stack<float> ex_elapsedTime)
    {
        ...
        {
            occuredBursts = formations[i].formation_Base.GetFormation().SetUp(ref occuredBursts, ref ex_elapsedTime);
    ...
    public override Depth CalculateNesting(ref Stack<int> occuredBursts, Depth count)
    {
        ...
        for (int i = 0; i < formations.Length; i++)
        {
            ...
            nestingCount = formations[i].formation_Base.GetFormation().CalculateNesting(ref occuredBursts, new Depth() { layerDepth = 0, nestDepth = 0 });
        ...
        GlobalPooling pooling, float2 position, ref Stack<float> ex_elapsedTime, bool reversed)
    {
        ...
        for (int i = 0; i < formations.Length; i++)
        {
            ...
            if (!isCompleted && !formations[i].formation_Base.GetFormation().IsComplete(ref occurredBursts))
            {
                var modifiedPos = position + formations[i].positionOffset;
                formations[i].formation_Base.GetFormation().UpdateFormation(layerMask, ref occurredBursts, elapsedTime, pooling, modifiedPos, ref ex_elapsedTime, 
            ...
            nestingCount = formations[i].formation_Base.GetFormation().CalculateNesting(ref occurredBursts, new Depth() { nestDepth = 0, layerDepth = 0 });
        }
